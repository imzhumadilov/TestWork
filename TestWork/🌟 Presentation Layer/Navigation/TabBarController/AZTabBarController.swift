...
//    open var notificationBadgeAppearance: BadgeAppearance = BadgeAppearance()
    ...
    /// The UIFont for the buttons. // fixfixfix
//    open var font: UIFont? = Barlow.bold.font(size: 10.0) {
//        didSet{
//            updateInterfaceIfNeeded()
//        }
//    }
...
//                button.addBadge(text: text, appearance: notificationBadgeAppearance)
    ...
    open func setTitle(_ text: String?, atIndex index: Int, titleColor: UIColor? = nil){
        if let buttons = buttons{
            if index < buttons.count{
                ...
                else { color = selectedColor ?? .black }
    ...
    private func setupButtons(){
        ...
        let maskPath = UIBezierPath(roundedRect: self.buttonsContainer.bounds,
                                    byRoundingCorners: [.topLeft, .topRight],
                                    cornerRadii: CGSize(width: 10.0, height: 10.0))
        let maskLayer = CAShapeLayer()
        maskLayer.frame = self.buttonsContainer.bounds
        maskLayer.path = maskPath.cgPath
        self.buttonsContainer.clipsToBounds = false
        self.buttonsContainer.layer.mask = maskLayer
        self.buttonsContainer.layer.shadowPath = maskPath.cgPath
        self.buttonsContainer.layer.shadowColor = AppTheme.backgroundShadow.cgColor
        self.buttonsContainer.layer.shadowOffset = CGSize(width: 0.0, height: 2.0)
        self.buttonsContainer.layer.shadowOpacity = 0.7
        self.buttonsContainer.layer.shadowRadius = 6.0
    }
...
//            button.titleLabel?.font = font // fixfixfix
    ...
    private func moveToController(at index:Int,animated:Bool){
        ...
        if let controller = controllers[index], !(animated && isAnimating) {
            ...
            self.statusBarStyle = index == 0 ? .lightContent : .default
